...
function updateMetaTags(film) {
    ...
    // Atualiza a meta descrição principal
    document.getElementById('meta-description').setAttribute('content', description);

    // Atualiza as meta tags do Open Graph (Facebook, WhatsApp)
    document.getElementById('og-title').setAttribute('content', newTitle);
    document.getElementById('og-description').setAttribute('content', description);
    document.getElementById('og-image').setAttribute('content', imageUrl);
    document.getElementById('og-url').setAttribute('content', pageUrl);

    // Atualiza as meta tags do Twitter
            ...
            <div class="ods-flip-container">
                <a href="${bnccLink}" target="_blank" rel="noopener noreferrer" class="ods-flipper-link" title="Competência ${comp.id}: ${comp.titulo} - Saiba mais na BNCC">
                    <div class="ods-flipper">
                        <div class="ods-front" style="background-image: url('bncc_icons/bncc_${comp.id}.svg'); background-size: cover; background-position: center;"></div>
                        <div class="ods-back bncc-card" data-bncc-number="${comp.id}"><p>${comp.descricao}</p></div>
                    </div>
                </a>
            </div>`).join('');
...
function renderFilmData(film) {
    ...
    let additionalInfoContent="";let hasAnyAdditionalInfo=!1;
    if(film.festivais){additionalInfoContent+=`<div class="filme-subsection"><h4><i class="fas fa-ticket-alt"></i> Festivais</h4><p>${film.festivais.replace(/\n/g,"<br>")}</p></div>`;hasAnyAdditionalInfo=!0}
    if(film.premios){additionalInfoContent+=`<div class="filme-subsection"><h4><i class="fas fa-award"></i> Prêmios</h4><p>${film.premios.replace(/\n/g,"<br>")}</p></div>`;hasAnyAdditionalInfo=!0}
    let otherDetailsContent="";
    if(film.audiodescricao){otherDetailsContent+=`<p><strong><i class="fas fa-assistive-listening-systems"></i> Audiodescrição:</strong> ${film.audiodescricao}</p>`;hasAnyAdditionalInfo=!0}
    if(film.closedCaption){otherDetailsContent+=`<p><strong><i class="fas fa-closed-captioning"></i> Closed Caption:</strong> ${film.closedCaption}</p>`;hasAnyAdditionalInfo=!0}
    if(film.legendasOutras){otherDetailsContent+=`<p><strong><i class="fas fa-language"></i> Outras Legendas:</strong> ${film.legendasOutras}</p>`;hasAnyAdditionalInfo=!0}
    if(film.website){const websiteUrl=film.website.startsWith("http")?film.website:`https://${film.website}`;otherDetailsContent+=`<p><strong><i class="fas fa-globe"></i> Website:</strong> <a href="${websiteUrl}" target="_blank">${film.website}</a></p>`;hasAnyAdditionalInfo=!0}
    if(otherDetailsContent){additionalInfoContent+=`<div class="filme-subsection"><h4><i class="fas fa-info-circle"></i> Outros Detalhes</h4>${otherDetailsContent}</div>`}
...
async function loadFilmData() {
    try {
        ...
        //======================================================================================
        // CHAMADA DA FUNÇÃO NO LOCAL CORRETO
        //======================================================================================
